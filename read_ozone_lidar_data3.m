...
folderPath='C:\Users\OzoneLidar\Documents\Lidar Data\20211118\txt';
...
aodfile='C:\Users\OzoneLidar\Documents\MATLAB\ozone lidar\AeronetAoddata\20210923_20211111_CCNY.txt';
...
% id=3;
% figure
% plot(log(pz2_287(:,id)),hkm,'r');hold on;
% plot(log(pz2_299(:,id)),hkm,'b');
% legend('287nm','299nm')
...
% id=12;
% figure
% plot(pz2_287_avg(:,id),hkm,'r');hold on;
% plot(pz2_299_avg(:,id),hkm,'b');
% legend('287nm','299nm')
% xlabel('Range corrected signal (a.u.)');
% ylabel('Altitude (km)');
...
for i=1:N
  ...
  diff(1:dh1)=diff1(1:dh1);
  diff(dh1+1:dh2)=diff2(dh1+1:dh2);
  diff(dh2+1:end)=diff3(dh2+1:end);
  N_O3_SG(:,i)=1/(2*d_sigma).*diff;
end
...
for i=1:length(t_chm15k)
  ...
  N_O3_bsc_SG(:,i)=1/(2*d_sigma).*diff;
end
