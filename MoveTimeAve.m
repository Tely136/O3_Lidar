...
function [DataTimeAve,TimeIndArr]=MoveTimeAve(DataMat,lenHeight,TimeLen,T)
...
for i=1:n% every 16 points average and store in a new matrix
    ind=(i-1)*T;
    DataTimeAve(:,i)=mean(DataMat(:,ind+1:ind+T),2,'omitnan');
    TimeIndArr(i)=uint32(ind+floor((1+T)/2));
    if i==n
    DataTimeAve(:,i)=mean(DataMat(:,ind+1:ind+T+diff),2,'omitnan');
    end
end
