...
folder_path='/Users/Tinker/Documents/MATLAB/ozonelidar/ozonelidar_repo/O3dial_data/20221021/txt/';
save_path='/Users/Tinker/Documents/MATLAB/ozonelidar/ozonelidar_repo/ozone_lidar_results/20221021/';
nbin=8000;
dzm=3.75;
bgbins=100;
td=280;
...
% OLfileName='ol220521_1316_1736.mat';load([save_path,OLfileName]);
load(OLfileName)
...
Th_cld2=0.007;
grad_bg =-0.002;
[cldNum, cldBaseZ, cldTopZ,cldBaseZ_ind,...
    cldTopZ_ind,cldBase_qc_flag,cldTop_qc_flag,...
    cld_mask,pz2,d_Pz2]=cld_detect2(cld_screen_prof,cld_start_bin,...
    cld_end_bin,hkm_nr,Th_cld1,Th_cld2, grad_bg);
%% cloud screen test
% id=30;
...
plot(new_sigprof.an287nr(:,id),hkm_nr,'r--');
plot(new_sigprof.an299nr(:,id),hkm_nr,'b--');
set(gca,'XScale','log')
legend('far-287','far-299','near-287','near-299');
xlabel('mV');
ylabel('Altitude (km)');
...
snr_299 = sqrt(12000/40*(new_sigprof.merge299merge).^2./(new_sigprof.merge299merge+bg299));
snr_287 = sqrt(12000/40*(new_sigprof.merge287merge).^2./(new_sigprof.merge287merge+bg287));
...
%% option1 use iterative method to directly retrieve the 299nm signal profile
Sm= 8*pi/3;
zsurf=0.3;
zref=4;
Rc=1.005;  % reference value for the total-backscatter to molecular backscatter ratio
Sa= 45;
absc_299_ol=nan(size(ND_O3_corr));
N_O3_bsc_ol=nan(size(absc_299_ol));
D_aext_ol=nan(size(absc_299_ol));
movnum1=21;% 78.5m 0-2km 1:533
movnum2=51;% 191.25m  2-5km 534:1333
movnum3=81;% 303.75m  >5km  1334:end 
ND_O3_corr_sm = sglp_smooth(ND_O3_corr,1,h1_hkm,h2_hkm,movnum1,movnum2,movnum3);

ND_O3_all_corr=ND_O3_corr;
for i =1:length(TimeInHour_avg)
    % check if there is clouds within the calculation range
    if cldBaseZ(1,i)<zref
        continue
    end
    prof_fr_nr=new_sigprof.merge299fr(:,i);
    no3_init_m3 = ND_O3_corr_sm(:,i); % the O3 ND with only molecular correction as initial O3 ND
    [absc_299_ol(:,i),N_O3_bsc_ol(:,i),D_aext_ol(:,i),ND_O3_all_corr(:,i),err]=OL299_aero_iter_retr(no3_init_m3,...
        am299,prof_fr_nr,hkm_nr,zsurf,zref,Rc,Sa);
end


%% option2 using ceilometer data 
aodfile='/Users/Tinker/Documents/MATLAB/ozonelidar/AOD/20220501_20220727_CCNY.txt';
...
NDplot(N_O3_bsc./NDAir_m3*1e9,TimeInHour_avg,hkm_nr,title_str,y_limit,[-10,10],fontsize);
hold on
plot(chm_t(ind_t),chm15k.pbl_aero(ind_t),'*');legend('PBLH')

NDplot(N_O3_bsc_ol./NDAir_m3*1e9,TimeInHour_avg,hkm_nr,title_str,y_limit,[-10,10],fontsize);
hold on
plot(chm_t(ind_t),chm15k.pbl_aero(ind_t),'*');legend('PBLH')
...
title_str = [datestr(DateTime_avg(1),'yyyy/mm/dd'),' CCNY-DIAL ozone mixing ratio (ppb)'];
NDplot(O3ppb_sm,TimeInHour_avg,hkm_nr,title_str,[0.35,6.5],[0,100],14);
hold on;
plot(TimeInHour_avg,cldBaseZ(1,:),'o'); legend('CldBase')

O3ppb = ND_O3_all_corr./NDAir_m3*1e9;
title_str = [datestr(DateTime_avg(1),'yyyy/mm/dd'),' CCNY-DIAL ozone mixing ratio (ppb), with aerosol correction'];
NDplot(O3ppb,TimeInHour_avg,hkm_nr,title_str,[0.35,6.5],[0,100],14);
hold on;
plot(TimeInHour_avg,cldBaseZ(1,:),'o'); legend('CldBase')
...
NDplot(movmean(chm15k.aero_bsa1064(:,ind_t),3,2),chm_t(ind_t),chm_h,title_str,[0.25,7],[0,2e-3],12);
hold on
plot(chm_t(ind_t),chm15k.pbl_aero(ind_t),'*');
legend('PBLH')

% aerosol backscatter coefficient 
title_str = ['299nm retrieved aerosol backscatter (km^{-1}sr^{-1})',datestr(DateTime_avg(1),'yy/mm/dd')];
NDplot(1e3*absc_299_ol,TimeInHour_avg,hkm_nr,title_str,[0.25,7],[0,4e-3],fontsize);
hold on
plot(chm_t(ind_t),chm15k.pbl_aero(ind_t),'*');legend('PBLH')
