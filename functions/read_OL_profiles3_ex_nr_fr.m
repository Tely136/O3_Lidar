...
%%
function [OLfileName,save_path]=read_OL_profiles3(folder_path,save_path,nbin,dzm,bgbins,td,nAvg,sib_hkm)
% 
%
...
%     end 
    profile_287_an_raw(:,k)= Data.data(:,5);
    profile_299_an_raw(:,k)= Data.data(:,7);
    profile_287_nr_an_raw(:,k)= Data.data(:,1);
    profile_299_nr_an_raw(:,k)= Data.data(:,3);
% PC signal
    profile_287_pc_raw(:,k)= Data.data(:,6);
    profile_299_pc_raw(:,k)= Data.data(:,8);
    profile_287_nr_pc_raw(:,k)= Data.data(:,2);
    profile_299_nr_pc_raw(:,k)= Data.data(:,4);   
...
%% Test the SIB
% figure
% plot(hkm,profile_287_pc_corr(:,k),'b--','LineWidth',1);hold on
% plot(hkm,sib_fitpc(:,k),'g-','LineWidth',1); 
% plot(hkm,profile_287_pc_sib(:,k),'k-','LineWidth',0.7);
% plot(hkm,profile_287_pc(:,k),'m-','LineWidth',0.7)
% set(gca,'YScale','log')
% xlabel('Altitude (km)');
% ylabel('PC signal (MHz)');
% legend('287pc-not bg subtr','exponential fit bg','287pc-exp bg subtr','287pc-const bg subtr')
