%
%% Description: 
% Merge the far range and near range signal using linear fit with cloud
% screening 
%% Input
...
function prof_merge = merge_nr_fr_prof_cldscr(prof_nr,prof_fr,start_merge_hkm,end_merge_hkm,hkm_fr,hkm_nr,z1,z2,cldn,cbz,ctz)
...
for i=1:len_t
    cond1= cldn(i)==0;
    cond2= (cldn(i)>0 && (z2<cbz(i,1) || (ctz(i,1))>z1) ); % not overlaping with first cloud
%     cond3= cldn(i)==2 && ...
%            sum(( hkm_nr>z1&hkm_nr<z2)&((hkm_nr>cbz(i,1)&hkm_nr<ctz(i,1))|(hkm_nr>cbz(i,2)&hkm_nr<ctz(i,2))))==0;
