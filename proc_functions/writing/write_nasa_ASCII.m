function write_nasa_ASCII(fname,folder,config)
    fileID = fopen(fullfile(folder,fname), 'w');
    
    header_lines(fileID,fname,config)
    write_lines(fileID,config)
    fwrite(fileID,newline);
    
    write_data(fileID,config);
    fwrite(fileID,newline);
...
function write_data(fid,config)
    
    % TODO: make this work for any number of bins and when there are datasets with different numbers of bins in them 
    for k = 1:8000
        line = "";
        for i = 1:16
            config_tr = config.TR(i);
            if config_tr.enable == 1
                an_data = scale_binary_analog(config_tr.data(k,1),config_tr.range,config_tr.adcbits,config.acquired_shots);    
                pc_data = scale_binary_pc(config_tr.data(k,2),config_tr.mhz,config.acquired_shots);
    
                line = line + sprintf("%0.5f %0.5f ",an_data,pc_data);
            end
        end
        fwrite(fid,line);
        fwrite(fid,newline);
...
function write_lines(fid,config)
    shots = config.acquired_shots;
    ...
    for i = 1:16
        ...
        if config_tr.enable == 1
            ...
            an_line = ds_line(config_tr.id,config_tr.enable,0,1,bins,0,config_tr.pmtv,config_tr.binWidth,config_tr.wavelength,'o',config_tr.binShift,0,config_tr.adcbits,shots,an_range,'BT');
            pc_line = ds_line(config_tr.id,config_tr.enable,1,1,bins,0,config_tr.pmtv,config_tr.binWidth,config_tr.wavelength,'o',config_tr.binShift,0,config_tr.adcbits,shots,config_tr.discr,'BC');
